...
<head>
  ...
  <script>
    function updateCount() {
      ...
      msgCount.textContent = message.value.length + "/1000";
    }
...
<div class="max-w-4xl mx-auto p-6">
  ...
  <c:if test="${status ne 'Closed'}">
    <form action="${pageContext.request.contextPath}/tickets/message" method="post"
      ...
      <input type="hidden" name="ticketId" value="${ticketId}" />
      <label class="block text-sm font-medium text-gray-700">Your message (<span id="msgCount">0/1000</span>)</label>
      <textarea name="message" id="message" required rows="4" maxlength="1000"
                oninput="updateCount()"
                class="w-full rounded-lg border px-3 py-2 shadow-sm focus:ring-2 focus:ring-blue-500"
