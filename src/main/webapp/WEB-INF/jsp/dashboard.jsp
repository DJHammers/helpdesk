  ...
  <aside class="w-64 bg-white border-r flex flex-col justify-between">
    <div>
      ...
      <nav class="mt-6 space-y-1">
        <a href="${pageContext.request.contextPath}/dashboard?view=tickets"
           class="flex items-center px-6 py-2 text-sm font-medium rounded-lg hover:bg-gray-100 ${view=='tickets'?'bg-gray-100':''}">
          View Tickets
        </a>
        <c:if test="${isAdmin}">
          <a href="${pageContext.request.contextPath}/dashboard?view=users"
             class="flex items-center px-6 py-2 text-sm font-medium rounded-lg hover:bg-gray-100 ${view=='users'?'bg-gray-100':''}">
            Manage Users
          </a>
        </c:if>
      </nav>
    ...
    <div class="p-6">
      <a href="${pageContext.request.contextPath}/logout"
         class="block w-full text-center py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
        Sign Out
      </a>
    </div>
  </aside>
  ...
  <main class="flex-1 p-8 overflow-auto">
    <c:choose>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <c:forEach var="u" items="${usersList}">
                <tr>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${u.id}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${u.username}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${u.email}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${u.role}</td>
      ...
      <c:otherwise>
        ...
        <form method="get" action="${pageContext.request.contextPath}/dashboard" class="mb-6 flex flex-wrap gap-4">
          <input type="hidden" name="view" value="tickets"/>
          <div class="flex items-center space-x-2">
            <label for="status" class="text-sm font-medium text-gray-700">Status:</label>
            <select id="status" name="status" onchange="this.form.submit()"
                    class="block rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Assign To</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <c:forEach var="t" items="${ticketsList}">
                <tr class="hover:bg-gray-50 cursor-pointer"
                  ...
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${t.id}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${t.username}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${t.subject}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${t.status}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${t.assignedRole}</td>
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">
                    <c:if test="${role=='Admin' or role=='Support'}">
                      <form action="${pageContext.request.contextPath}/tickets/assign"
                            method="post"
                  ...
                  <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">${t.createdAt}</td>
                </tr>
              </c:forEach>
