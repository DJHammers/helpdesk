            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <c:forEach var="t" items="${ticketsList}">
                <tr class="hover:bg-gray-50 cursor-pointer"
                    onclick="if (!event.target.closest('form') && !event.target.closest('select') && !event.target.closest('button')) window.location='${pageContext.request.contextPath}/tickets/view?id=${t.id}';">
                  ...
                  <td class="px-4 py-2 text-sm text-gray-700">
                    <c:if test="${role=='Admin' or role=='Support'}">
                      <form action="${pageContext.request.contextPath}/tickets/assign" method="post"
                        ...
                        <select name="role" onclick="event.stopPropagation()"
                  ...
                  <td class="px-4 py-2 text-sm text-gray-700">
                    <fmt:formatDate value="${t.createdAt}" pattern="yyyy-MM-dd HH:mm:ss"/>
                  </td>
                </tr>
              </c:forEach>
