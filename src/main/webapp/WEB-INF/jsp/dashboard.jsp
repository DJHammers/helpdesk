            ...
            <c:otherwise>
              ...
              <form method="get" action="${pageContext.request.contextPath}/dashboard"
                ...
                <select id="status" name="status" onchange="this.form.submit()"
                  ...
                  <c:if test="${isAdmin}">
                    <option value="ASSIGNED_Admin" ${statusFilter=='ASSIGNED_Admin' ?'selected':''}>Assigned to Admin
                    </option>
                  </c:if>
                  <c:if test="${!isAdmin}">
                    <option value="ASSIGNED_Support" ${statusFilter=='ASSIGNED_Support' ?'selected':''}>Assigned to
                      Support</option>
                  </c:if>
                </select>
              </form>
              <div class="overflow-hidden rounded-lg border bg-white shadow">
                <table class="min-w-full divide-y divide-gray-200">
                  <thead class="bg-gray-50">
                    <tr>
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">User
                      </th>
                      ...
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Assigned</th>
                      <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Assign
                        To</th>
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <c:forEach var="t" items="${ticketsList}">
                      <tr class="hover:bg-gray-50 cursor-pointer"
                        ...
                        <td class="px-4 py-2 text-sm text-gray-700">${t.username}</td>
                        ...
                        <td class="px-4 py-2 text-sm text-gray-700">${t.createdAt}</td>
                      </tr>
                    </c:forEach>
